...
import styles from "./basket.module.scss"; // Импорт CSS модулей для стилизации компонента
import { BasketContext } from '../../components/contex/basketContext'; // Импорт контекста корзины
...
export const Basket = ({
  ...
  const { basket, setBasket } = useContext(BasketContext); // Извлечение состояния корзины и функции для обновления корзины из контекста
  const [counts, setCounts] = useState(basket.map(() => 1)); // Инициализация состояния для хранения количества каждого товара
  ...
  const handleRemove = (index) => {
    const newBasket = basket.filter(function(element, i) { // Фильтрация корзины, удаление товара по индексу
    ...
    setBasket(newBasket); // Обновление состояния корзины
  ...
  const totalPrice = counts.reduce((sum, count, index) => sum + count * basket[index].discont_price, 0); // Подсчет общей стоимости товаров в корзине
      ...
      <section className={styles.wrapper}> {/* Основной контейнер для корзины */}
        <div className={styles.container}> {/* Контейнер для списка товаров */}
          <div className={styles.card}> {/* Карточка с товарами */}
            <ul>
              {basket.map(({ image, price, discont_price, title }, index) => ( // Перебор товаров в корзине
                <section key={index} className={styles.mainSection}> {/* Секция для каждого товара */}
                  ...
                  <div className={styles.countItemPriceConteiner}> {/* Контейнер для названия, цены и количества товара */}
                    <h3 className={styles.title}>
                      ...
                      <button className={styles.closeModul} onClick={() => handleRemove(index)}>X</button> {/* Кнопка для удаления товара */}
                    </h3>
