import React, { useCallback, useEffect, useState } from "react"; // Импорт библиотеки React и хуков (useEffect, useState)
...
export const Product = () => { // Объявление функционального компонента Product
  ...
  const fetchData = useCallback(async () => { // Функция для выполнения запроса данных о продукте
    try {
  ...
  }, [id]); // Объявление зависимостей useCallback
  ...
  }, [fetchData]); // Зависимость useEffect пустая, что указывает на то, что эффект нужно выполнить только при монтировании компонента
