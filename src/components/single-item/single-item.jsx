...
import { BasketContext } from '../../components/contex/basketContext'; // Импорт контекста корзины для добавления товаров в корзину
...
export const SingleItem = ({
  ...
  const { addItemToBasket } = useContext(BasketContext); // получение функции добавления товара в корзину из контекста
    ...
    <div className={styles.wrapper}> {/* основной контейнер компонента */}
      ...
      <div className={styles.allInformationWrapper}> {/* контейнер для всей информации о товаре */}
        ...
        <div className={styles.counterWrapper}> {/* контейнер для счетчика количества товара */}
          ...
          <button className={styles.addToCart} onClick={handleAddToBasket}> {/* кнопка добавления товара в корзину */}
            Add to cart
          </button>
        </div>
